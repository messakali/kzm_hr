...
class EBds(models.TransientModel):
    ...
    def generate(self):
        ...
        for rec in self:
            ...
            file_content_decoded = str(base64.decodebytes(rec.file_import))
            # print(file_content_decoded)
            # print(str(file_content_decoded))
            # print(type(file_content_decoded))
            # print(type(str(file_content_decoded)))
            ...
            for emp in data[2:-2]:
                ...
                if bultain_id:
                    ...
                    if e_bds_sortant_id:
                        e_bds_sortant_line_id = e_bds_sortant_line.search(
                            [('e_bds_sortant_id', '=', e_bds_sortant_id.id),
                    ...
                    S_CTR = str(int(salaire_reel * 100) + int(emp[16:25]) + int(N_SALAIRE_Plaf) + int(
                        working_days) + situation_chiffre)
                    ...
                    salaire_pre = ""
                    salaire_pre = "B02" + emp[3:105] + N_AF_A_REVERSER + working_days.rjust(2,
                                                                                            '0') + salaire_reel_str + N_SALAIRE_Plaf + L_situation + S_CTR.replace(
                        '.', '').rjust(19, '0') + L_filter
                    output += salaire_pre + '\n'
                # Sortants:
                else:
                    ...
                    if e_bds_sortant_id:
                        ...
                        if e_bds_sortant_line_id:
                            ...
                            salaire_sortant = ""
                            salaire_sortant = "B02" + emp[3:105] + "".rjust(30,
                                                                            '0') + situation + S_CTR_sortant + "".rjust(
                                104)
                            output += salaire_sortant + '\n'
            NT1 = "".rjust(42, '0')

            B03 = "B03" + l2[3:16] + str(nombre_salaries).rjust(6, '0') + NT1 + str(total_matriculation).rjust(15,
                                                                                                               '0') + "".rjust(
                12, '0') + str(total_working_days).rjust(6, '0') + str(total_salaire_reel).rjust(15, '0') + str(
                total_N_SALAIRE_Plaf).rjust(13, '0') + str(total_S_CTR).rjust(19, '0') + "".rjust(116)
            output += B03 + '\n'
            # Entrants
            ...
            for entrant in bultain_ids:
                ...
                salaire_entrant = "B04" + l2[3:16] + str(N_Num_Assure).rjust(9, '0') + str(L_Nom_nom).ljust(30) + str(
                    L_Nom_prenom).ljust(30) + str(L_Num_CIN).rjust(8, " ") + str(N_Nbr_Jours).rjust(2, '0') + str(
                    N_Sal_Reel).rjust(13, '0') + str(N_Sal_Plaf).rjust(9, '0') + str(S_Ctr).rjust(19, '0') + "".rjust(
                    124)
                output += salaire_entrant + '\n'

            if not bultain_ids:
                salaire_entrant = "B04" + l2[3:16] + ''.rjust(9) + str('').rjust(60) + str('').rjust(8, " ") + str(
                    0).rjust(2, '0') + str(0).rjust(13, '0') + str(0).rjust(9, '0') + str(0).rjust(19, '0') + "".rjust(
                    124)
                output += salaire_entrant + '\n'

            B05 = "B05" + l2[3:16] + str(N_Nbr_Salaries_entrants).rjust(6, '0') + str(N_T_Num_Imma_entrants).rjust(15,
                                                                                                                   '0') + str(
                N_T_Jours_Declare_entrants).rjust(6, '0') + str(N_T_Salaire_Reel_entrants).rjust(15, '0') + str(
                N_T_Salaire_Plaf_entrants).rjust(13, '0') + str(N_T_Ctr_entrants).rjust(19, '0') + "".rjust(170)
            output += B05 + '\n'
            ...
            B06 = "B06" + l2[3:16] + str(N_Nbr_Salaries).rjust(6, '0') + str(N_T_Num_Imma).rjust(15, '0') + str(
                N_T_Jours_Declares).rjust(6, '0') + str(N_T_Salaire_Reel).rjust(15, '0') + str(N_T_Salaire_Plaf).rjust(
                13, '0') + str(N_T_Ctr).rjust(19, '0') + "".rjust(170)
            output += B06 + '\n'
            out = base64.encodebytes(output)
            rec.state = 'get'
            rec.file_export = out
