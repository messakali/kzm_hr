...
class HrRubriqueSettings(models.TransientModel):
    ...
    def create_rubrique(self):
        ...
        while i < 25:
            ...
            if rubrique_id:
                ...
                i += 1
        hsup_25 = self.env['hr.payroll_ma.rubrique'].sudo().browse(self.env.ref('hr_payroll_ma.hsup_25').id)
        hsup_50 = self.env['hr.payroll_ma.rubrique'].sudo().browse(self.env.ref('hr_payroll_ma.hsup_50').id)
        hsup_100 = self.env['hr.payroll_ma.rubrique'].sudo().browse(self.env.ref('hr_payroll_ma.hsup_100').id)
        jrs_chomes_payes = self.env['hr.payroll_ma.rubrique'].sudo().browse(self.env.ref('hr_payroll_ma.jrs_chomes_payes').id)
        jrs_conges_payes = self.env['hr.payroll_ma.rubrique'].sudo().browse(self.env.ref('hr_payroll_ma.jrs_conges_payes').id)
    ...
    def create_parametre(self):
        ...
        parametre_id = self.env.ref('hr_payroll_ma.parametres_data8')
        if parametre_id:
    ...
    def create_journal(self):
        ...
        jouranl_id = self.env.ref('hr_payroll_ma.salary_journal')
        if jouranl_id:
