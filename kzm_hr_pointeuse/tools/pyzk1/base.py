                        ...
                        if self.__firmware == 6:
                            while len(userdata) >= 28:
                                uid, privilege, password, name, card, group_id, timezone, user_id = unpack(
                                    'HB5s8s5sBhI', userdata.ljust(28)[:28])
                                password = unicode(password.split('\x00')[0], errors='ignore')
                                name = unicode(name.split('\x00')[0], errors='ignore').strip()
                                card = unpack('Q', card.ljust(8, '\x00'))[0]  # or hex value?
                                group_id = str(group_id)
                            ...
                            while len(userdata) >= 72:
                                uid, privilege, password, name, separator, group_id, user_id = unpack(
                                    'Hc8s24s4sx7sx24s', userdata.ljust(72)[:72])
                                # u1 = int(uid[0].encode("hex"), 16)
                                # u2 = int(uid[1].encode("hex"), 16)
                                # uid = u1 + (u2 * 256)
                                privilege = int(privilege.encode("hex"), 16)
    ...
    def get_attendance(self):
        ...
        if self.__firmware == 6:
            ...
            while len(attendance_data) >= 40:
                uid, user_id, sparator, timestamp, status, space = unpack('H24sc4scv', attendance_data.ljust(40)[:40])
