...
class KzmBadge(models.Model):
            ...
            for machine_id in machine_ids:
                ...
                if machine_id.connection_state == True:
                    try:
                        ...
                        msg, state = record.add_user(machine_id.id, 
                            record.uid, str(record.employee_id.name), int(record.privilege),
                                  str(record.password), str(record.groupid or ''), str(record.userid), int(record.cardnumber))
                        if state:
    ...
    def write(self, vals):
        ...
        if not not_upadte_badge and (vals.get('matricule', False) or vals.get('pointeuse_ids', False) or vals.get('password', False) or vals.get('cardnumber', False)):
            for this in self:
