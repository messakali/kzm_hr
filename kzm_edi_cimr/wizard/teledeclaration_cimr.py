    ...
    def generate(self):
        ...
        for rec in self:
            ...
            for line in rapport_cimr.rep_cimr_line_ids:
                ...
                nom_complet = (line.employee_id.name.upper()).replace(' ', '') + '' + (
                    line.employee_id.prenom.upper()).replace(' ', '')
                l = str(code) + str(company.cimr or "") + str(company.cimr_category)[:2].rjust(2, '0') + str(cimr) \
                    ...
                    + str(date_fin) + str(cin) + str(
                    cnss) + '00000000000000' + '00000000000000000000000000000000000' + str(rapport_cimr.trimestre) \
                    + str(rapport_cimr.annee)
                print(l)
                output += l + '\n'
